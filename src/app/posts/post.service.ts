...
export class PostService {
  ...
  getPost():Observable<Post[]>{
    return this.http.get<Post[]>(this.urlpost2 + '/all', {headers: this.agregarAuthorizationHeader()});
  }
  create(post:Post):Observable<Post>{
    return this.http.post(this.urlpost2 + '/create', post)
    .pipe(
  ...
  getPostId(id:number): Observable<Post>{
    return this.http.get<Post>(`${this.urlpost2}/${id}`).pipe(
      catchError(e=>{
  ...
  update(post:Post):Observable<any>{
    return this.http.put<any>(`${this.urlpost2}/${post.idpost}`, post).pipe(
      catchError(e =>{
        if(e.status == 400){
          return throwError(e);
        }
  ...
  delete(id:number):Observable<Post>{
    return this.http.delete<Post>(`${this.urlpost2}/${id}`).pipe(
      catchError(e =>{
        if(e.error.mensaje){
          console.error(e.error.mensaje);
        }
